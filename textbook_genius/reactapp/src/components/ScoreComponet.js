...
const ScoreComponent=({relation})=>{
    ...
    async function handleYes(){
        ...
        if (yes) {
            ...
            // 进行点赞
            console.log("yes"+yes);
            const success = await handleRequest("http://8.130.18.80:80/api/up-score-user", "POST", body);
            if (success) {
                setYes(true);
                setYesCount(yesCount+1)
            }
    ...
    async function handleNo(){
        ...
        if(no){
            // 取消点踩
            const success = await handleRequest("http://8.130.18.80:80/api/down-score-user", "DELETE", body);
            if (success) {
                setNo(false);
            ...
            //进行点踩
            const success = await handleRequest("http://8.130.18.80:80/api/down-score-user", "POST", body);
            if (success) {
                setNo(true);
                setNoCount(NoCount+1)
            }
