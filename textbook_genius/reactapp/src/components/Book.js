...
const Book=({relation})=>{
    ...
    async function getBookDetails(){
        let res=await fetch("http://8.130.18.80:80/api/get-book"+"?isbn="+isbn)
        if(!res.ok){
            navigate("/");
            return;
        }
    ...
    const getMarkDetail =async ()=>{
        ...
        let response=await fetch("http://8.130.18.80:80/api/get-mark-status"+"?userid="+getCookie('user_id')+"&bookisbn="+isbn,{
            credentials:'include'
        });
        if(!response.ok){
            return;
    ...
    async function handleMark(){
        ...
        if (mark) {
            // 取消收藏
            console.log("取消收藏")
            const success = await handleRequest("http://8.130.18.80:80/api/mark-book", "DELETE", body);
            if (success) {
                setMark(false);
            }
        } else {
            // 进行收藏
            console.log("进行收藏")
            const success = await handleRequest("http://8.130.18.80:80/api/mark-book", "POST", body);
            if (success) {
                setMark(true);
            }     
        }
